...
def register() -> rx.Component:
    ...
    rx.center(
        rx.vstack(
            rx.text("Inicio / Mi Cuenta / ", rx.text("Crear Cuenta", as_="span", font_weight="bold")),
            rx.text("Comprá más rápido y llevá el control de tus pedidos, ¡en un solo lugar!", font_size="13px", margin_bottom="8px"),
            rx.box(
                rx.text("NOMBRE", font_weight="bold"),
                rx.input(placeholder="ej: María Pérez", width="100%",color="black"),
                rx.text("EMAIL", font_weight="bold", margin_top="10px"),
                rx.input(placeholder="ej: tunombre@email.com", width="100%",color="black"),
                rx.text("TELÉFONO (OPCIONAL)", font_weight="bold", margin_top="10px"),
                rx.input(placeholder="ej: 1123445567", width="100%",color="black"),
                rx.text("CONTRASEÑA", font_weight="bold", margin_top="10px"),
                rx.hstack(
                    rx.input(type_="password", placeholder="********", width="100%",color="black"),
                    rx.icon(tag="eye-off"),
                ),
                rx.text("CONFIRMAR CONTRASEÑA", font_weight="bold", margin_top="10px"),
                rx.hstack(
                    rx.input(type_="password", placeholder="********", width="100%",color="black"),
                    rx.icon(tag="eye-off"),
                ),
                rx.button("CREAR CUENTA", bg="black", color="white", width="100%", margin_top="15px",_hover={"transform": "scale(1.05)", "cursor": "pointer"}),
                align="start",
                width="100%",
                max_width="400px",
            ),
            rx.text("¿Ya tenés una cuenta? ", rx.link("Iniciá sesión", href="/login", color="black", font_weight="bold")),
            spacing="3",
            align="center",
            justify="center",
            padding_bottom="40px",
